...
function getConfig() {
    ...
    //备选配置文件
    const configBackupFile = path.resolve(pkgDir, '.preparecommitrc');
    if (fs.existsSync(configFile)) {
        return JSON.parse(fs.readFileSync(configFile, 'utf-8') || '{}');
    } else if (fs.existsSync(configBackupFile)) {
        return JSON.parse(fs.readFileSync(configBackupFile, 'utf-8') || '{}');
    }
    return {};
}
